...
public class ContainerSavingHelper {
    ...
    public static void loadAllItems(String tag, ValueInput input, NonNullList<ItemStack> items) {
        loadAllItems(tag, input, items, _ -> {});
    }

    public static void loadAllItems(ValueInput input, NonNullList<ItemStack> items, Consumer<ItemStack> failure) {
        loadAllItems(TAG_ITEMS, input, items, failure);
    }

    public static void loadAllItems(String tag, ValueInput input, NonNullList<ItemStack> items, Consumer<ItemStack> failure) {
        for (var stack : input.listOrEmpty(tag, CODEC)) {
            if (stack.isValidInContainer(items.size())) {
                items.set(stack.slot(), stack.stack());
            } else {
                failure.accept(stack.stack());
            }
        }
    }

    public static void loadAllItems(ValueInput input, Consumer<ItemStack> items) {
        loadAllItems(TAG_ITEMS, input, items);
    }

    public static void loadAllItems(String tag, ValueInput input, Consumer<ItemStack> items) {
        for (var stack : input.listOrEmpty(tag, CODEC)) {
            items.accept(stack.stack());
        }
    }
}